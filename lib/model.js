...
const CONNECTION_STRING_OPTIONS = {
  ...
  connectTimeoutMS: { type: 'number', default: 120000 },
  socketTimeoutMS: { type: 'number', default: 120000 },
  ...
  serverSelectionTimeoutMS: { type: 'number', default: 120000 },
  serverSelectionTryOnce: { type: 'boolean', default: undefined },
  heartbeatFrequencyMS: { type: 'number', default: 120000 },
  /**
...
assign(props, {
   ...
   * Enable ssh agent.
   */
  sshTunnelAgent: { type: 'string', default: '/run/user/1000/gnupg/S.gpg-agent.ssh' },
  /**
...
assign(derived, {
  ...
  sshTunnelOptions: {
    ...
    fn() {
      ...
      console.warn('building opts for ssh');

      const opts = {
        readyTimeout: 120000,
        forwardTimeout: 120000,
      ...
      if (this.sshTunnel === 'USER_PASSWORD') {
        ...
        /* eslint no-sync: 0 */
        if (this.sshTunnelAgent) {
          console.warn('using ssh agent');
          opts.agent = this.sshTunnelAgent;
        } else if (this.sshTunnelIdentityFile && this.sshTunnelIdentityFile[0]) {
