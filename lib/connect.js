...
const getTasks = (model, setupListeners) => {
  ...
  model.hosts.forEach(item => {
    assign(tasks, {
        'Create SSH Tunnel for host: ${item.host}': cb => {

            if (model.sshTunnel === 'NONE') {
                return ctx.skip('The selected SSH Tunnel mode is NONE.');
            }

            let modelClone = cloneDeep(model, true);
            modelClone.set({
                hostname: item.host,
                port: model.port
            });
            const ctx = status('Create SSH Tunnel for host: ${item.host}', cb);
            tunnels.push(createSSHTunnel(modelClone, ctx));
  ...
  assign(tasks, {
    'Connect to MongoDB': cb => {
      ...
      if (model.sshTunnel !== 'NONE') {
        ...
        modelClone.set({
          hosts: tunnels
          .map(item => {return {host: item.options.localAddr, port: item.options.localPort}})
        });
        debug('data-service using ssh tunnel, using hostname: ' + JSON.stringify(modelClone.hosts));
      }
      debug(modelClone.driverUrl);
